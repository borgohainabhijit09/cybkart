            ...
            <nav className="container mx-auto px-6">
                <div className="flex items-center justify-between h-20">
                    ...
                    <div className="hidden lg:flex items-center space-x-8">
                        ...
                        <Link
                            href="/contact"
                            className="bg-[#0F172A] text-white px-5 py-2.5 rounded-lg font-medium text-sm hover:bg-slate-800 transition-all shadow-md hover:shadow-lg transform hover:-translate-y-0.5"
                        >
                            Book Strategy Call
                        </Link>
                    </div>
                ...
                {/* Mobile Menu */}
                {mobileMenuOpen && (
                    <div className="lg:hidden absolute top-20 left-0 right-0 bg-white border-t border-slate-200 shadow-xl p-6 space-y-4 h-[calc(100vh-5rem)] overflow-y-auto">
                        <Link href="/" className="block text-slate-700 font-medium py-2">Home</Link>
                        <Link href="/about" className="block text-slate-700 font-medium py-2">About Us</Link>

                        <div className="space-y-1">
                            <div className="font-semibold text-slate-900 pb-2 border-b border-slate-100 mb-2">Services</div>
                            {services.map((service) => (
                                <Link key={service.href} href={service.href} className="block pl-4 py-3 text-slate-600 text-sm border-l-2 border-transparent hover:border-blue-500 hover:bg-slate-50">
                                    {service.name}
                                </Link>
                            ))}
                        </div>

                        <Link href="/case-scenarios" className="block text-slate-700 font-medium py-2">Case Scenarios</Link>
                        <Link href="/knowledge-hub" className="block text-slate-700 font-medium py-2">Knowledge Hub</Link>

                        <div className="pt-4">
                            <Link href="/contact" className="block w-full bg-[#0F172A] text-white text-center px-5 py-3 rounded-lg font-medium shadow-md">
                                Book Strategy Call
                            </Link>
                        </div>
                    </div>
